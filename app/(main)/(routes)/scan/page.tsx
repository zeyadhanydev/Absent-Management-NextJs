...
export default function QRScannerPage() {
  ...
  const [userLogin] = useState<string>("ZizO-lol");
  ...
  useEffect(() => {
    const initializeFingerprint = async () => {
      try {
        ...
        toast.error('Failed to generate device fingerprint. Please refresh the page.', {
          duration: 5000,
          position: 'top-center',
        });
      }
  ...
  useEffect(() => {
    ...
    getCurrentLocation()
      ...
      .catch(err => {
        ...
        toast.error('Location services are required. Please enable GPS and refresh.', {
          duration: 6000,
          position: 'top-center',
          icon: '📍',
        });
        ...
        try {
          ...
          if (err.response) {
            if (err.response.data && err.response.data.message) {
              ...
              errorMessage = 'You are not authorized to verify attendance in this class.';
            } else if (err.response.status === 404) {
              errorMessage = 'QR code not found or class/section does not exist.';
            }
          } else if (err.request) {
          ...
          if (errorMessage.includes('enrolled') || errorMessage.includes('section')) icon = '🧑‍🎓';
      ...
      <div className="space-y-4">
        ...
        <div className="text-sm text-center space-y-1">
          <p className="text-gray-600">
            <span className="font-medium">🕒 UTC Time:</span><br />
            {timestamp}
          </p>
          {location && (
        ...
        {/* Status indicator */}
        {scanResult && (
          <div className={`text-center p-2 rounded-md ${showConfetti ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
            <p className="font-medium">
              {showConfetti ? 'Verification Successful!' : 'Awaiting verification...'}
            </p>
          </div>
        )}
        ...
        <div className="space-y-2">
          {!scanning && !scanResult && (
            ...
            >
              {!location || !fingerprint ? 'Waiting for services...' : 'Start Camera'}
            </Button>
          )}
          {scanning && (
        ...
        {/* Result display */}
        {scanResult && (
          <div className={`border rounded-md p-4 ${showConfetti ? 'bg-green-50' : 'bg-gray-100'} space-y-2`}>
            <h3 className="text-sm font-medium">Scan Details:</h3>
            <div className={`text-xs ${showConfetti ? 'bg-green-100' : 'bg-gray-200'} p-2 rounded-md space-y-1`}>
              <p><strong>Code:</strong> {scanResult.split('/')[6] || scanResult}</p>
              <p><strong>UTC Time:</strong> {timestamp}</p>
              <p><strong>User:</strong> {userLogin}</p>
              {location && (
                <p><strong>Location:</strong> {location.latitude.toFixed(6)}, {location.longitude.toFixed(6)}</p>
              )}
              <p><strong>Device ID:</strong> {fingerprint.slice(0, 8)}...</p>
            </div>
          </div>
        )}
